...
def _reshape_output_torch(tensor: Tensor, b: _I, c: _I) -> Tensor:
    """Reshape the output tensor to the original dimension
...
def _ssim_torch(
    ...
    if data_range != 255.0:
        warnings.warn(f"`data_range` must is 255.0!")
        data_range = 255.0
    ...
    # If image size larger than 256, downsample
...
def _msssim_torch(
        ...
        downsampling: bool = False,
        get_ssim_map: bool = False,
        get_cs_map: bool = True,
        get_weight: bool = False,
        prod_out: bool = True,
        only_test_y_channel: bool = True,
        data_range: float = 255.0,
) -> [Tensor, Tensor]:
    """PyTorch implements MS-SSIM (Multi-scale Structural Similarity, structural similarity) function
    ...
    Args:
        ...
        downsampling (bool): Whether to perform downsampling, default: ``False``
        get_ssim_map (bool): Whether to return SSIM image, default: ``False``
        get_cs_map (bool): whether to return CS image, default: ``True``
        get_weight (bool): whether to return the weight image, default: ``False``
        prod_out (bool): Whether to return the product result, default: ``True``
        only_test_y_channel (bool): Whether to test only the Y channel of the image, default: ``True``
        data_range (float, optional): Maximum value range of images. Default: 255.0

    Returns:
        ms_ssim_metrics (Tensor): MS-SSIM metrics
    """
    ...
    # Calculate the SSIM map of the original image
    if get_cs_map is False:
        get_cs_map = True

    # Define SSIM weights
    weights = torch.FloatTensor([0.0448, 0.2856, 0.3001, 0.2363, 0.1333]).to(raw_tensor.device, dtype=raw_tensor.dtype)
    weights_level = weights.shape[0]
    mcs = []

    ssim_metrics = None
    for _ in range(weights_level):
        ssim_metrics, cs_map = _ssim_torch(
            raw_tensor,
            dst_tensor,
            gaussian_kernel_window,
            downsampling,
        ...
        mcs.append(cs_map)
        padding = (raw_tensor.shape[2] % 2, raw_tensor.shape[3] % 2)
        raw_tensor = F_torch.avg_pool2d(raw_tensor, kernel_size=(2, 2), padding=padding)
        dst_tensor = F_torch.avg_pool2d(dst_tensor, kernel_size=(2, 2), padding=padding)

    mcs = torch.stack(mcs, dim=0)

    if prod_out:
        ms_ssim_metrics = torch.prod((mcs[:-1] ** weights[:-1].unsqueeze(1)), dim=0) * (ssim_metrics ** weights[-1])
    else:
        weights = weights / torch.sum(weights)
        ms_ssim_metrics = torch.sum((mcs[:-1] * weights[:-1].unsqueeze(1)), dim=0) + (ssim_metrics * weights[-1])

    return ms_ssim_metrics
...
class MSSSIM(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            get_cs_map (bool): whether to return CS image, default: ``True``
            get_weight (bool): whether to return the weight image, default: ``False``
            prod_out (bool): Whether to return the product of MS-SSIM, default: ``True``
            crop_border (int, optional): how many pixels to crop border. Default: 0
            only_test_y_channel (bool, optional): Whether to test only the Y channel of the image. Default: ``True``
    ...
    def forward(self, raw_tensor: Tensor, dst_tensor: Tensor) -> Tensor:
        ...
        ms_ssim_metrics = _msssim_torch(raw_tensor,
                                        dst_tensor,
                                        self.gaussian_kernel_window,
                                        self.downsampling,
                                        self.get_ssim_map,
