...
__all__ = [
    ...
    "file_filter", "make_directory", "make_divisible", "save_checkpoint", "AverageMeter", "ProgressMeter", "Summary",
]
...
def load_resume_state_dict(
        ...
        compile_state: bool,
        model_weights_path: str,
) -> tuple[Module, int, float, float, Optimizer]:
    ...
    Returns:
        model (nn.Module): the model after loading the model weights
        start_epoch (int): The starting node to resume training
        psnr (float): psnr when resuming training
        ssim (float): ssim when resuming training
        optimizer (nn.optim): The optimizer after loading the model weights
    """
    ...
    return model, start_epoch, psnr, ssim, optimizer
